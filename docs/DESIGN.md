...
| **"Doubles baseline inference cost"** | Estimate in source doc | **Measured, not estimated (see [studies/phase1-calibration.md](studies/phase1-calibration.md)):** the probe makes `n × (1 + axes)` calls, which at the usable minimum n≥5 is **10× for one axis and 20–32× for multi-axis** — well above both the "doubles" figure and the earlier 3–6× guess. Null-caching can amortize the baseline toward `n × axes` marginal. |
